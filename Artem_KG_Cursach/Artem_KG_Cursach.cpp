...
int WINAPI wWinMain(HINSTANCE _In_ hInstance, HINSTANCE _In_opt_ hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	w.lpszClassName = L"CompGraphics"; //имя программы - объявлено выше
	...
	HWND hWnd = CreateWindowW(
		L"CompGraphics", //Имя программы
		L"Курсовая работа", //Заголовок окна
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT messg, WPARAM wParam, LPARAM lParam) // Функция окна
{
	...
	case WM_PAINT: // сообщение рисования
		...
		hdc = BeginPaint(hWnd, &ps); // начинаем рисовать фигуры, все фигуры рисуются обычной линией, прописанной в файле заголовке Paint2.h
		...
		// закраска фигуры, в данном случае призмы по координатам 
		...
		// По аналогии с призмой, рисуем треугольную пирамиду, используя те же функции, что и на призме
		...
		// закраска фигуры в данном случае
	...
	case WM_KEYDOWN: // Обработка нажатия клавиши
		...
		case VK_LEFT: // Обрабатывает клавишу "Стрелка влево"
			move(prism, -5);
			move_pyramid(pyramid, -5);
			InvalidateRect(hWnd, NULL, TRUE);
			break;

		case VK_RIGHT: // Обрабатывает клавишу "Стрелка вправо"
			move(prism, 5);
			move_pyramid(pyramid, 5);
			InvalidateRect(hWnd, NULL, TRUE);
			break;

		case VK_UP: // Обрабатываем клавиу "стрелка вверх"
			move(prism, 0, -5);
			move_pyramid(pyramid, 0, -5);
			InvalidateRect(hWnd, NULL, TRUE);
			break;

		case VK_DOWN: // Обрабатывает клавишу "Стрелка вниз"
			move(prism, 0, 5);
			move_pyramid(pyramid, 0, 5);
			InvalidateRect(hWnd, NULL, TRUE);
			break;
		...
		case 0x53: // Обрабатывает клавишу S.
			...
			////////////////////////////////////////////////////////////
			///////////////////////////////////////////////////////////
		case VK_SUBTRACT: //  NUMPAD - (Уменьшение)
			scale(prism, 0.9);
			scale_pyramid(pyramid, 0.9);
			InvalidateRect(hWnd, NULL, TRUE);
			break;

		case VK_ADD: //  NUMPAD + (увеличение)
			scale(prism, 1.1);
		...
		case 0x58: // Движение по Х кооординате
			rotateX(prism, angle);
			rotateX_pyramid(pyramid, angle);
			InvalidateRect(hWnd, NULL, TRUE);
			break;

		case 0x59: // Движение по Y координате
			rotateY(prism, angle);
			rotateY_pyramid(pyramid, angle);
			InvalidateRect(hWnd, NULL, TRUE);
			break;

		case 0x5A: // Движение по Z координате
			rotateZ(prism, angle);
			rotateZ_pyramid(pyramid, angle);
			InvalidateRect(hWnd, NULL, TRUE);
			break;

		case VK_SPACE: //  (добавление контурных линий)
			if (kontur == 0)
			{
				kontur = 1;
			}
	...
	default:
		return(DefWindowProc(hWnd, messg, wParam, lParam)); //освобождаем очередь приложения (в данном случае, чтобы программа не переполнялась)
	}
	return 0;
}
...
//Движение фигур по координатам X Y Z
...
// Изменение в размерах фигур по матрц
